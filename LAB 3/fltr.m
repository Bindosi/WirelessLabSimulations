function frm = transmit(symbs_tx, flt_stp)   
%{ 
flt_stp.T: filter type. RC, RRC, G.
flt_stp.sps: over sampling ratio.
flt_stp.span: filter span.
flt_stp.alpha: Roll-off factor. For RC and RRC only.
flt_stp.BT: bandwidth-time product. For G only.
%}
    ...
    if strcmp(T, 'RRC')
        alpha = flt_stp.alpha;
        fltr = rcosdesign(alpha, span, sps, 'sqrt'); 
        smpls = upfirdn(symbs_tx, fltr, sps);
    elseif strcmp(T, 'RC')
        alpha = flt_stp.alpha;
        fltr = rcosdesign(alpha, span, sps, 'normal');
        smpls = upfirdn(symbs_tx, fltr,sps);
    elseif strcmp(T, 'G')
        BT = flt_stp.BT;
        fltr = gaussdesign(BT,span,sps);
        smpls = upfirdn(symbs_tx, fltr,sps);
    %elseif strcmp(T, 'R')
        %smpls = rectpulse(symbs, sps);
    end
    
    if strcmp(T, 'OQPSK')
        rTx = real(smpls);
        iTx = imag(smpls);
        rTx = [zeros(sps/2,1) ; rTx];
        iTx = [iTx; zeros(sps/2,1)];
        smpls = rTx + 1i * iTx;
    end
    
    frm = [zeros(2e2,1); smpls; zeros(2e2,1)];
end

function symbs = pi4qpsk(data)
    s1 = pskmod(data(1:2:end), 4);
    s2 = pskmod(data(2:2:end), 4, pi/4);
    symbs = [s1,s2].';
    symbs = symbs(:);
end
