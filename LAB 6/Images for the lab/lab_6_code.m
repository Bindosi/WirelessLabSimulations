...
% setting up receive object
        Rx = sdrrx('Pluto', 'SamplesPerFrame', hw_stp.N,...
            ...
            'CenterFrequency',hw_stp.F,...
            'BasebandSampleRate',hw_stp.R,...
            'GainSource',"AGC Fast Attack" ,... %AGC Fast Attack
...
release(Rx);
figure()
[Pxx_tx_q2,F_tx_q2] = pwelch(receve,[],[],[],hw_stp.R,'centered');
plot(F_tx_q2,10*log10(Pxx_tx_q2));
grid on
...
% Upsampling
 tx_frame_up_q7 = upsample(trasmit_frame_q7,flt_stp.sps);

% creatıng our fılter
filter = rcosdesign(flt_stp.alpha, flt_stp.span, flt_stp.sps);

% pefroming convolution to get the filtered frame
filteredFrame_q7 = conv(filter, trasmit_frame_q7);

% testing the look 
figure()
[Pxx_tx_q7,F_tx_q7] = pwelch(filteredFrame_q7.',[],[],[],hw_stp.R,'centered');
...
%[frm,symbs_tx] = transmit(Tx,mod_stp,flt_stp);
 Tx.transmitRepeat(filteredFrame_q7'); 

% Receiving
[smpls_rx_q9, prmb_rx_q9, smpls_dt_q9, symbs_rx_q9] = receive(Rx,mod_stp, flt_stp,hw_stp);

figure()
[Pxx_tx_q9,F_tx_q9] = pwelch(smpls_rx_q9,[],[],[],hw_stp.R,'centered');
...
% Receiving
[smpls_rx_q14, prmb_rx_q7, smpls_dt_q14, symbs_rx_q14] = receive(Rx,mod_stp, flt_stp,hw_stp);
