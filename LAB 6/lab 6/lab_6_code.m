...
hw_stp.F     = 2.35e9;
hw_stp.R     = 1e6;
hw_stp.Tx_ID = 'sn:104473541196000a07000d00f7831e1eb9'; %Yunus
%hw_stp.Tx_ID = 'sn:10447354119600060d001800cf281e583b';
%hw_stp.Rx_ID ='sn:104473dc599300131200210082672a4170';
hw_stp.Rx_ID ='sn:104473541196000cf7ff14008dd81e43f6'; %Yunus
hw_stp.N = 30e3;
...
baseband_frequency  = 1000;                               % Frequency of the tone (1 kHz)
sinusoidal_tone     = sin(2*pi*baseband_frequency*t) +1j*sin(2*pi*baseband_frequency*t);     % Generate a sine wave tone
...
% scatterplot(down)
% downforpre          =downsample(received_samples(52+length(guard_samples)*8:length(received_samples)-8*length(data_q14)-length(guard_samples)*8-49),8)
% scatterplot(downforpre)
...
estimated_h         = mean(prmb_rx_q14/Preamble_q14);    
...
eq_smpls_rx         = smpls_rx_q14.* conj(estimated_h(1));
...
for i               = 1:length(eq_smpls_rx)
    % Calculate Euclidean distance to each constellation point
    distances       = abs(eq_smpls_rx(i) - table);
    ...
    detected_data(i)    = mod(idx-1, mod_stp.M_q14);
end
