...
NumSymbols        = length(modulatedDataBits)/length(effectiveDataCarriers);
...
reshapedModulatedSymbols = reshape(modulatedDataBits,length(effectiveDataCarriers),length(zeropaddingMessage)/length(effectiveDataCarriers));

to_ifft(effectiveDataCarriers,:) = reshapedModulatedSymbols;
...
interPolated = interp1(pilotCarriers,estimatedOFDMChannel,2:1:length(fdomainSymbol)+1,"spline");
